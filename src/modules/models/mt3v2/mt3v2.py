...
class MT3V2(nn.Module):
    def __init__(self, params):
        ...
        self.prediction_space_dimensions = 2 # (xy1,xy2,xy3,xy4,vxy)  #2  # (x, y) position and velocity #Actual 5xy 5vxvy atm
        selectiondim=2
        ...
        self.bounding_box_predictor = MLP(params.arch.d_model,
                                        hidden_dim=params.arch.d_prediction_hidden,
                                        output_dim=self.prediction_space_dimensions,
                                        num_layers=params.arch.n_prediction_layers,)
        ...
        nn.init.xavier_uniform_(self.pos_vel_predictor.layers[-1].weight.data, gain=1.0)
        nn.init.constant_(self.pos_vel_predictor.layers[-1].bias.data, 0.1)
        nn.init.xavier_uniform_(self.bounding_box_predictor.layers[-1].weight.data, 0.1)
        nn.init.constant_(self.bounding_box_predictor.layers[-1].bias.data, 0.1)
        ...
        self.bounding_box_predictor = _get_clones(self.bounding_box_predictor, num_pred)
        ...
        self.decoder.bounding_box_predictor = self.bounding_box_predictor
        ...
        if self.two_stage:
            ...
            self.reference_points_linear = nn.Linear(params.arch.d_model, self.prediction_space_dimensions)
    ...
    def gen_encoder_output_proposals(self, embeddings, memory_padding_mask, normalized_xy_measurements, normalized_vxvy_measurements):
        # Compute presigmoid version of normalized measurements
        normalized_xy_measurements_presigmoid = torch.log(normalized_xy_measurements / (1 - normalized_xy_measurements))
        normalized_vxvy_measurements_presigmoid = torch.log(normalized_vxvy_measurements / (1 - normalized_vxvy_measurements))

        # Set to inf invalid measurements (masked or outside the FOV)
        output_xy_proposals_valid = ((normalized_xy_measurements > 0.01) & (normalized_xy_measurements < 0.99)).all(-1,keepdim=True)
        output_vxvy_proposals_valid = ((normalized_vxvy_measurements > 0.01) & (normalized_vxvy_measurements < 0.99)).all(-1,keepdim=True)
        output_proposals_valid = torch.logical_and(output_xy_proposals_valid, output_vxvy_proposals_valid)

        normalized_xy_measurements_presigmoid = normalized_xy_measurements_presigmoid.masked_fill(memory_padding_mask.unsqueeze(-1), float('inf'))
        normalized_xy_measurements_presigmoid = normalized_xy_measurements_presigmoid.masked_fill(~output_proposals_valid, float('inf'))
        
        normalized_vxvy_measurements_presigmoid = normalized_xy_measurements_presigmoid.masked_fill(memory_padding_mask.unsqueeze(-1), float('inf'))
        normalized_vxvy_measurements_presigmoid = normalized_xy_measurements_presigmoid.masked_fill(~output_proposals_valid, float('inf'))
        ...
        return projected_embeddings, normalized_xy_measurements_presigmoid, normalized_vxvy_measurements_presigmoid
    ...
    def get_two_stage_proposals(self, measurement_batch, embeddings):
        ...
        measurements = measurement_batch.tensors[:, :, :self.d_detections] #Skalar bort tid som är [4]
        ...
        vx = measurements[:, :, 1] * (measurements[:, :, 2].cos())
        vy = measurements[:, :, 1] * (measurements[:, :, 2].sin())
        xy_measurements = torch.stack([xs, ys], 2) 
        vxvy_measurements = torch.stack([vx, vy], 2)
        
        gated_x_measurements=[]
        gated_y_measurements=[]
        for bs in xy_measurements:
            gated_xs=[]
            gated_ys=[]
            for point in bs:
                gated_xy_meas = np.linalg.norm(point-bs,axis=1)
                gated_idx = np.where(gated_xy_meas < 5)[0]
                gated_xs.append(xy_measurements[len(gated_x_measurements)][gated_idx][:,0].sum()/len(gated_idx))
                gated_ys.append(xy_measurements[len(gated_x_measurements)][gated_idx][:,1].sum()/len(gated_idx))
            
            gated_x_measurements.append(gated_xs)
            gated_y_measurements.append(gated_ys)
        
        gated_xy_measurements = torch.stack([torch.from_numpy(np.array(gated_x_measurements)),torch.from_numpy(np.array(gated_y_measurements))], 2)

        #xy_measurements=gated_xy_measurements #Doom

        # Normalize measurements to 0.25 - 0.75 (to avoid extreme regions of the sigmoid)
        normalized_xy_meas = xy_measurements / self.output_scaling_factor[0] + 0.5
        normalized_vxvy_meas = vxvy_measurements / self.output_scaling_factor[1] + 0.5

        # Compute projected encoder memory + presigmoid normalized measurements (filtered using the masks)
        projected_embeddings, normalized_xy_meas_presigmoid, normalized_vxvy_meas_presigmoid = \
            self.gen_encoder_output_proposals(embeddings.permute(1, 0, 2), measurement_batch.mask, normalized_xy_meas, normalized_vxvy_meas)
        

        # Compute scores and adjustments and bounding boxes
        scores = self.decoder.obj_classifier[self.decoder.num_layers](projected_embeddings)
        #KASTA IN BEROENDE AV SNR HÄR?
        ...
        bounding_boxes = self.bounding_box_predictor[self.decoder.num_layers](projected_embeddings)
        ...
        normalized_meas_presigmoid = torch.cat((normalized_xy_meas_presigmoid, normalized_vxvy_meas_presigmoid,), dim=2)
        ...
        topk_adjusted_normalized_meas_presigmoid = torch.gather(adjusted_normalized_meas_presigmoid, 1, repeated_indices).detach()
        ...
        topk_bounding_boxes = torch.gather(bounding_boxes, 1, repeated_indices[:,:, :2]).detach()
        ...
        return object_queries, query_positional_encodings, topk_adjusted_normalized_meas, scores, adjustments, adjusted_normalized_meas, topk_bounding_boxes, bounding_boxes
    ...
    def forward(self, measurements: NestedTensor):
        ...
        if self.two_stage:
            (object_queries,
             ...
             topk_bounding_box_references,
             bounding_box_references) = self.get_two_stage_proposals(measurements, embeddings)
        else:
        ...
        # Feed embeddings and object queries to decoder #Får in 4 xy vxvy från selmech, vill spotta ut 10, kopiera samma 4ggr?
        result = self.decoder(object_queries, 
                              embeddings,
                              ...
                              reference_bounding_box = topk_bounding_box_references) #0.5 0.5 0.5 kolla på det
        intermediate_state_predictions_normalized, intermediate_uncertainties, intermediate_logits, intermediate_bounding_boxes, debug_dict = result

        # Un-normalize state predictions
        intermediate_state_predictions = intermediate_state_predictions_normalized - 0.5
        intermediate_state_predictions[:, :, :, :-2] *= self.output_scaling_factor[0] #Alla förutom 2 sista scaleas enligt FOV
        ...
        if self.two_stage:
            ...
            adjusted_meas[:, :, :-2] *= self.output_scaling_factor[0] #Alla förutom 2 sista scaleas enligt FOV
        ...
        # Pack output using standardized Prediction class #PREDICTIONS#PREDICTIONS#PREDICTIONS ÄNDRA HÄÄÄR
        prediction = Prediction(positions=intermediate_state_predictions[-1][:, :, :self.prediction_space_dimensions],
                                ...
                                bounding_boxes = intermediate_bounding_boxes[-1],
        ...
        intermediate_predictions = [Prediction(positions=p[:, :, :self.prediction_space_dimensions],
                                               ...
                                               bounding_boxes=b,
                                               uncertainties=u,
                                               logits=l) for p, l, u, b in zip(intermediate_state_predictions[:-1],
                                                                            intermediate_logits[:-1],
                                                                            intermediate_uncertainties[:-1],
                                                                            intermediate_bounding_boxes[:-1])] if self.return_intermediate else None
        encoder_prediction = Prediction(positions=adjusted_meas[:, :, :self.prediction_space_dimensions],
                                        ...
                                        bounding_boxes=bounding_box_references,
