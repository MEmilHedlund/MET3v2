...
from modules.contrastive_classifier import ContrastiveLoss
...
if __name__ == '__main__':
    ...
    if params.debug.enable_logging:
    ...
    if params.debug.enable_plot or params.debug.save_plot_figs:
        ...
        loss_ax.set_ylabel('Logits Loss', color='C0')
        loss_ax.grid('on')
        ...
        #if params.debug.save_plot_figs and params.debug.enable_logging:
    ...
    for i_gradient_step in range(params.training.n_gradient_steps):
        ...
        for i_split_step in range(n_splits):
            try:
                ...
                loss_dict, indices = mot_loss.forward(labels_pv, labels_bb, prediction, intermediate_predictions, encoder_prediction, loss_type=params.loss.type)

                total_loss = sum(loss_dict[k] for k in loss_dict.keys())
                logs = update_logs(logs, 'last_layer_losses', loss_dict[f'{params.loss.type}_logits'].item() + loss_dict[f'{params.loss.type}_state'].item() + loss_dict[f'{params.loss.type}_BB'].item())
        ...
        if (params.debug.enable_plot and i_gradient_step % params.debug.plot_interval == 0) or \
            ...
            output_ax.cla()
            output_ax.grid('on')
            output_truth_plot(trajectories , output_ax, prediction, labels_pv, labels_bb, indices, batch, params, training_example_to_plot=0)
            ...
            if params.debug.save_plot_figs and i_gradient_step % params.debug.save_plot_figs_interval == 0:
                ...
                if params.debug.enable_logging:
                    fig.savefig(os.path.join(logger.log_path, 'figs', filename))

                total_loss_line.set_data(x_axis, losses)
                total_loss_ax.relim()
                total_loss_ax.autoscale_view()
                if params.debug.enable_logging:
                    total_loss_fig.savefig(os.path.join(logger.log_path, 'figs', filename))

                if params.loss.contrastive_classifier and params.debug.enable_logging:
                    contrastive_loss_fig.savefig(os.path.join(logger.log_path, 'figs', filename))
                if params.loss.false_classifier and params.debug.enable_logging:
                    false_loss_fig.savefig(os.path.join(logger.log_path, 'figs', filename))
                if params.debug.enable_logging:
                    state_uncertainties_fig.savefig(os.path.join(logger.log_path, 'figs', filename))
        ...
        # Save checkpoint
        if (i_gradient_step+1) % params.training.checkpoint_interval == 0 and params.debug.enable_logging:
            filename = f'checkpoint_gradient_step_{i_gradient_step}'
            folder_name = os.path.join(logger.log_path, 'checkpoints')
            save_checkpoint(folder=folder_name,
                            filename=filename,
        ...
        if params.debug.evaluate_gospa_interval is not None and \
            ...
            gospaeval.append(gospa_results) #Logga alla resultat över tid
            plt.plot([item[0] for item in gospaeval]) #Plotta vald gospaparameter
            if params.debug.enable_logging:
                ...
                print('metrics/gospa_total', gospa_results[0], i_gradient_step)
                print('metrics/gospa_localization_error', gospa_results[1], i_gradient_step)
                print('metrics/gospa_localization_error_normalized', gospa_results[2], i_gradient_step)
                print('metrics/gospa_missed', gospa_results[3], i_gradient_step)
                print('metrics/gospa_false', gospa_results[4], i_gradient_step)
